  ...
  const getItem = (collectionName:string,targetItemId:string) => {
    return new Promise((resolve, reject) => {
        ...
        const unsubscribe = onSnapshot(collectionRef, (querySnapshot) => {
          ...
          const targetedData=base?.data?.find((item:Item)=>item.id ==targetItemId);
          if (targetedData) {
...
//adding new items
const addItems=async(collectionName:string,newValue:object,documentName:string)=>{
//   return new Promise((resolve, reject) => {
//     const collectionRef = collection(db, collectionName);
...
//deleting item 
const deleteItem = async (collectionName: string, targetItemId: string,documentName:string) => {
  try {
      const collectionRef = collection(db, collectionName,);
    ...
    console.log(mappedData);
      ...
      const updatedData = base.data.filter((item: Item) => item.id !== targetItemId);
    console.log(updatedData);
    
      // Update the document with the modified data
      const docRef = doc(db, collectionName, documentName)
      await setDoc(doc(db, collectionName, documentName),  {data:updatedData}, { merge: true });
...
return { getItems,getItem,addItems,deleteItem};
